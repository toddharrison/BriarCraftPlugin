...
public class ZownManager implements IZownManager {
        ...
        if (!zownMap.containsKey(name)) {
            ...
            if (!intersectsExistingZown(rootTree, zown)) {
                ...
                if (player == null || player.isOperator()
                        || player.safeHasPermission("zown.admin.manage.create")
                        || flag != null && flag) {
    ...
    public boolean removeZown(final World world, final String name, final Player player) {
        ...
        if (zownMap != null && zownMap.containsKey(name)) {
            ...
            if (zownTree != null && player == null || player != null
                    && (player.isOperator() || player.safeHasPermission("zown.admin.manage.delete")
    ...
    public boolean renameZown(final World world, final String oldName, final String newName,
        ...
        if (zownMap != null && zownMap.containsKey(oldName) && !zownMap.containsKey(newName)) {
            ...
            if (zownTree != null && player == null || player != null
                    && (player.isOperator() || player.safeHasPermission("zown.admin.manage.rename")
    ...
    @Override
    public boolean resizeZown(final World world, final String name, final Point p1,
            final Point p2) {
    ...
    public boolean resizeZown(final World world, final String name, final Point p1, final Point p2,
        ...
        if (zownMap != null) {
            ...
            if (zownTree != null && player == null || player != null
                    && (player.isOperator() || player.safeHasPermission("zown.admin.manage.resize")
                            || zownTree.getData().isOwner(player))) {
                final Tree<Zown> rootTree = zownTrees.get(world);
                if (!intersectsExistingZown(rootTree, p1, p2, zownTree)) {
                    final Tree<Zown> targetTree = getTargetContainingZown(rootTree, p1, p2,
                            zownTree);
    ...
    private boolean intersectsExistingZown(final Tree<Zown> zownTree, final Point p1,
            final Point p2, final Tree<Zown> ignoredZown) {
        boolean intersects = false;
        for (final Tree<Zown> t : zownTree) {
    ...
    private Tree<Zown> getTargetContainingZown(final Tree<Zown> tree, final Point p1,
            final Point p2, final Tree<Zown> ignoredZown) {
        Tree<Zown> containingZown = null;
